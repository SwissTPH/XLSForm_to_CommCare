...
Foreach-Object {
	...
	if ($regex_data_look) {
		...
		$replace_underline_hint = 'hint"><value> $1 $2 $3'
		...
		$regex_markdown = '(?ms)<text id="((/[^/|^"]+)+/(?:label|_help)_[^"]+?label)">(<value form="image">[^<]+</value>)*<value>'
	...
	# remove the decimal-date-time which is not supported and not required in Commcare
		...
		(Get-Content  -Encoding UTF8 -Path $_.FullName -Raw ) `
			...
			| Out-File -Encoding UTF8 ('odk.' + $fileName + '.xml')
